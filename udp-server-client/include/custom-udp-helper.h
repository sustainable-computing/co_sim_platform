 ...
 * 
 * Modelled after udp-echo-helper.h
 */
...
public:
  /**
   * Create CustomUdpServerHelper which will make life easier for people trying
  ...
  /**
   * Create a CustomUdpServer application on the specified Node.
   *
  ...
  /**
   * Create a CustomUdpServer application on specified node
   *
   ...
   *
   * Create one custom udp server application on each of the Nodes in the
   * NodeContainer.
   *
...
private:
  /**
   * Install an ns3::CustomUdpServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an CustomUdpServer will be installed.
...
public:
  /**
   * Create CustomUdpClientHelper which will make life easier for people trying
   ...
   */
  CustomUdpClientHelper (Address ip, uint16_t port);
  /**
   * Create CustomUdpClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  CustomUdpClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a CustomUdpClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be CustomUdpClient).
  ...
  /**
   * Create a custom udp client application on the specified node.  The Node
   * is provided as a Ptr<Node>.
   *
   * \param node The Ptr<Node> on which to create the CustomUdpClient application.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the CustomUdpClient application
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
   ...
   * \param c the nodes
   *
   * Create one custom udp client application on each of the input nodes
   *
...
private:
  /**
   * Install an ns3::CustomUdpClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an CustomUdpClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
