...
CustomUdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CustomUdpClient")
    ...
    .AddAttribute ("PacketSize", "Size of echo data in outbound packets",
                   UintegerValue (100),
                   MakeUintegerAccessor (&CustomUdpClient::SetDataSize,
                                         &CustomUdpClient::GetDataSize),
                   MakeUintegerChecker<uint32_t> ())
...
void 
CustomUdpClient::SetDataSize (uint32_t dataSize)
{
...
uint32_t 
CustomUdpClient::GetDataSize (void) const
{
  NS_LOG_FUNCTION (this);
  return m_size;
}
...
void 
CustomUdpClient::SetFill (std::string fill)
{
  NS_LOG_FUNCTION (this << fill);

  uint32_t dataSize = fill.size ();
...
void 
CustomUdpClient::SetFill (uint8_t fill, uint32_t dataSize)
{
...
void 
CustomUdpClient::SetFill (uint8_t *fill, uint32_t fillSize, uint32_t dataSize)
{
...
void 
CustomUdpClient::ScheduleTransmit (Time dt)
{
  NS_LOG_FUNCTION (this << dt);
  m_sendEvent = Simulator::Schedule (dt, &CustomUdpClient::Send, this);
}
...
CustomUdpClient::Send (void)
{
  ...
  // Event might expire when failed to send due to error?
  ...
  if (m_dataSize)
    {
      ...
      //
      NS_ASSERT_MSG (m_dataSize == m_size, "CustomUdpClient::Send(): m_size and m_dataSize inconsistent");
      NS_ASSERT_MSG (m_data, "CustomUdpClient::Send(): m_dataSize but no m_data");
...
void
CustomUdpClient::HandleRead (Ptr<Socket> socket)
{
