...
from config.reader.ConfigErrors import InvalidNetworkType, NodeWithNetworkIdAlreadyExistsInNetwork, \
    ...
    NodeTooFarAwayFromAccessPoint, NetworkNotConnected
...
from config.reader.ValidationFunctions import check_distance_between_wifi_access_point_and_node,  check_network_is_connected_graph
...
class Config:
    ...
    def __read_network_connections(self, network_layout_dict):
        ...
        for network_conn_dict in network_layout_dict:
            ...
            neighbours = adj_list.get(network_connection.nodes[0], set())
            neighbours.add(network_connection.nodes[1])
            adj_list[network_connection.nodes[0]] = neighbours
            neighbours = adj_list.get(network_connection.nodes[1], set())
            neighbours.add(network_connection.nodes[0])
            adj_list[network_connection.nodes[1]] = neighbours 

        check_network_is_connected_graph(adj_list)         
