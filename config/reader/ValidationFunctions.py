...
from config.reader.ConfigErrors import InvalidNetworkType, InvalidAccessPointValueForNICType, \
    ...
    InvalidNetworkConnectionType, InvalidAppConnectionType, NetworkNotConnected
...
def check_network_is_connected_graph(adj_list):
    visited = set()
    first_node = next(iter(adj_list))
    visited.add(first_node)
    __dfs(first_node, visited, adj_list)
    print("LOL")
    print(visited)
    print(len(visited))
    print(adj_list.keys())
    print(len(adj_list.keys()))
    if len(visited) != len(adj_list.keys()):
        print("NOOOOO")


def __dfs(node, visited, adj_list):
    for neighbour in adj_list[node]:
        if neighbour not in visited:
            visited.add(neighbour)
            __dfs(neighbour, visited, adj_list)
    return visited
